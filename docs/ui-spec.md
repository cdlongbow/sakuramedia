...
- 下载任务卡片优先展示本地影片标题与封面，缺失时回退任务名与图片占位；提供暂停、恢复和删除操作，列表分页复用底部加载更多反馈
...
桌面端标签页（`/desktop/library/tags`）是“多选标签 + 影片列表”的单页集成形态，复用影片页的筛选与网格组件，当前仅实现桌面端（Web 端复用桌面路由）：

- 顶部为标签选择卡 `_TagSelectorPanel`（feature 内私有组件，位于 `desktop_tags_page.dart`）：
  - 顶部标题 `选择标签` + 右侧已选数量
...
- 下方为影片区：
  ...
  - 选择 ≥1 个标签后展示 `MovieFilterToolbar`（状态 / 合集类型 / 排序，与影片页一致）+ 总数（`X 部`）+ `MovieSummaryGrid` + 底部分页反馈，结构与影片页完全一致
...
路由 `/desktop/library/clip-collections/:collectionId`。头部为合集名（旁带编辑铅笔）+ 切片数 + 视图切换图标按钮 + 「添加切片」「播放」按钮；下方为切片内容，支持「列表 / 网格」两种排布，默认列表（仅在合集非空时显示视图切换与「播放」）：
...
- 索引器 Tab：Jackett API Key 配置、索引器条目管理、每个索引器与下载器的绑定关系，以及基于已保存配置的真实连通性测试
...
- 媒体维护 Tab：失效媒体复查与清理；切到该 Tab 后才懒加载，复用 `DesktopMediaMaintenancePage`，自带滚动触底加载更多
...
- PornBox 影片：`video-imports` 标签端点（`/video-imports`），`task_key = video_directory_import`；两类导入端点同构（列表分页、详情、失败文件重导/删除/重命名），PornBox 额外支持导入时归入合集

页面结构（每个标签内）：

- 顶部说明卡：左侧文案 + 右侧「刷新」「新建导入」按钮（`AppContentCard` + `AppButton`）
- 下方为导入作业卡片列表（按作业 id 倒序），滚动触底自动加载更多（转发给当前激活标签的 controller），底部复用 `AppPagedLoadMoreFooter`；无作业时用 `AppEmptyState`，初次加载用 `CircularProgressIndicator`
...
- 作业 `failed_count > 0` 时卡片提供「查看失败文件」展开，按需懒加载作业详情
- 失败条目按 `kind` 区分：仅 `file`（可处理）在作业终态下提供「重导 / 重命名 / 删除」行内操作，并在顶部提供「重导全部失败」；`skipped` / `warning` / `job` 仅展示原因，不提供操作
