...
class MediaImportController extends ChangeNotifier
  ...
  // 作业列表分页状态。
  List<ImportJobListItemDto> _jobs = const <ImportJobListItemDto>[];
  int _nextPage = 1;
  bool _hasMore = false;
  ...
  // 作业详情（失败文件）懒加载缓存。
  final Map<int, ImportJobDto> _details = <int, ImportJobDto>{};
  final Set<int> _detailLoading = <int>{};
  final Map<int, String> _detailErrors = <int, String>{};

  // task_run 实时进度表（仅保留媒体导入任务），按 task_run id 索引。
  ...
  Future<void> loadFirstPage() async {
    ...
    _notify();
    try {
      final response = await _mediaImportApi.listImportJobs(
        page: 1,
        pageSize: pageSize,
      );
  ...
  Future<void> ensureDetail(int jobId, {bool force = false}) async {
    ...
    _notify();
    try {
      final detail = await _mediaImportApi.getImportJob(jobId);
  ...
  Future<String?> renameFailedFile(
    ...
    try {
      final detail = await _mediaImportApi.renameFailedFile(
        jobId,
        path: path,
        newName: newName,
      );
  ...
  void _replaceJob(ImportJobListItemDto job) {
    final index = _jobs.indexWhere((item) => item.id == job.id);
    if (index < 0) {
      return;
    }
    final next = List<ImportJobListItemDto>.from(_jobs);
    next[index] = job;
    _jobs = next;
  }
    ...
    if (!_streamSeeded) {
      try {
        ...
        for (final run in bootstrap.activeTaskRuns) {
          if (run.taskKey == kMediaImportTaskKey) {
