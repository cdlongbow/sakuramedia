...
桌面端/Web 端“所有订阅女优最新影片”能力（“女优上新”）当前在“发现”页顶部以预览区块呈现（位于“今日推荐”之上，卡片支持订阅/取消订阅与上下文菜单，并与全局订阅状态同步）；三个桌面预览接口均请求 24 条作为缓冲，“女优上新”“今日推荐”在大屏最多显示 10 列、“推荐时刻”最多显示 6 列，三者均最多显示当前自适应网格的两行；窗口放大或侧栏折叠后会自动补足各行，完整结果进入右上角“更多”页面查看；点击区块右上角“更多”以 `push` 进入完整女优上新页面（`/desktop/library/follow`，顶栏返回“发现”）。该能力不再作为侧边栏一级入口，也不在桌面概览页复刻移动端顶部 Tab 的“关注”结构。
...
- `/mobile/settings/indexers` 已实现为真实索引器页：页面顺序为 Torznab 概览卡、连通性测试卡、索引器卡片列表；每个索引器可独立配置 API Key（可选，留空则不携带 apikey），新增 / 编辑通过底部抽屉即时提交；删除使用确认抽屉；单个索引器可绑定多个下载入口，PT 仅允许 qBittorrent，BT 可同时绑定 qBittorrent 与 115；如果任一绑定下载器已失效，会在卡片和详情抽屉中明确提示“绑定下载器已失效，请重新选择”
- `/mobile/settings/indexers` 的连通性测试卡提供“测试 Torznab 连通性”：仅使用已保存配置调用 `GET /indexer-settings/test` 做真实搜索；健康、零候选和失败都会在卡内保留查询、检查索引器数、候选数、耗时及修复提示
...
- 下方网格卡片
- 滚动触底加载更多
- 筛选条件点击后立即更新控件与摘要，已有内容保持可见；结果在 250ms 尾随防抖后异步替换
- 筛选更新失败时不回滚条件：有旧结果则在列表头下显示失败与重试，无旧结果则显示明确的失败空态
- 加载更多失败时保留内容并显示底部重试条

移动端影片筛选抽屉的「快捷筛选」区包含以下预设；发行年份区块同样在桌面浮层与移动抽屉共用：

- `全部`：回到默认筛选
- `最新订阅`：状态筛选固定为 `已订阅`，合集类型固定为 `单体`，排序方式固定为 `订阅时间`
- `最新入库`：状态筛选固定为 `可播放`，合集类型固定为 `单体`，排序方式固定为 `最近入库`
- 快捷预设与完整筛选项共用同一套 `status`、`collection_type`、`sort` 参数；抽屉没有“确定”按钮，点选后控件状态立即生效，服务端结果走统一防抖更新
...
- 主影片库与标签影片页在前端生成从当前年到 2008 年的年份 chip，不展示数量；默认最多显示两行，可展开全部年份。选择年份后影片列表请求追加 `year`
- 女优详情页年份选项仍来自 `GET /actors/{actor_id}/years`，chip 显示为 `年份(数量)`，例如 `2026(18)`；同样最多显示两行，可展开全部年份
...
- 下方为影片区：
  ...
  - 选择 ≥1 个标签后展示影片顶栏 `AppListHeader`（状态 / 合集类型 / 年份 / 排序，与影片页一致：桌面就地浮层、移动底部抽屉）+ 总数（`X 部`）+ `MovieSummaryGrid` + 底部分页反馈，结构与影片页完全一致
  ...
  - **标签选择区不进顶栏筛选入口**：它是本页的主维度导航，保持为列表上方的常驻面板；顶栏那个筛选入口只管影片的状态 / 合集 / 年份 / 排序
  - 影片请求走 `GET /movies?tag_ids=1,2,3`，多标签为 **OR** 语义（命中任一标签即选中）；筛选项沿用 `status`、`collection_type`、`year`、`sort`，标签页不接入 `special_tag`
...
- 播放器缩略图固定按图片长边 `512` 个物理像素解码，不随列数、侧栏宽度或设备 DPR 改变；全局 Flutter 图片缓存预算为 `128 MiB`，以便列数切换时复用同一批解码结果
...
- 索引器 Tab：Torznab 索引器条目管理、每个索引器独立的可选 API Key、与多个下载器的绑定关系（PT 禁止绑定 115），以及基于已保存配置的真实连通性测试
...
- **移动端视频列表**：视频区使用 `AppListHeader`，左侧筛选图标打开排序底部抽屉，中间显示当前排序与总数，右侧「选择」仅控制视频列表；进入选择态后顶栏原地显示退出、已选数量与全选，批量加入合集 / 删除仍保留在底部操作栏。合集区的「新建 / 查看全部」保持在合集标题行，首屏加载时使用与实际合集卡同尺寸的 `CollectionCardSkeleton` 横滑骨架。
- **PornBox 首屏加载**：桌面与移动端的视频合集横滑区均使用 `CollectionCardSkeletonRow`（内部复用 `CollectionCardSkeleton`）；移动端视频区使用 `AppMobileSkeletonList`，桌面视频网格使用 `AppCoverCardSkeleton`。已有内容静默刷新时不回退到骨架屏。
...
- 服务端筛选页把 `FilterUpdateState`、是否有旧结果和重试回调传给 `filterUpdate` / `hasPreviousFilterItems` / `onRetryFilter`；顶栏下方由 `AppFilterUpdateBar` 统一表达结果追赶状态
...
- 空数据：`AppEmptyState`
- 服务端筛选：UI 同步更新，结果请求统一 250ms 尾随防抖；已有内容不回退骨架屏，连续操作只提交最终条件，晚到旧响应不写回
- 筛选请求失败：不回滚筛选；有旧结果时显示 `AppFilterUpdateBar` 失败条并保留内容，无旧结果时用 `AppEmptyState` 展示失败和重试
- 首次请求失败：页面空态与重试；加载更多失败：保留内容并显示底部重试条
- 小型操作成功 / 失败：toast
