...
part 'media_import_provider.g.dart';

const String kMediaImportTaskKey = 'media_directory_import';

typedef MediaImportState =
    ImportJobsViewState<ImportJobListItemDto, ImportJobDto>;

/// JAV 媒体导入：分页作业、详情缓存、操作与 task_run SSE 的唯一状态源。
@Riverpod(retry: kNoAsyncNotifierRetry)
class MediaImport extends _$MediaImport
    with AsyncNotifierDisposeGuardMixin<MediaImportState>
  ...
  MediaImportState get _current =>
      state.value ?? MediaImportState(isInitialLoading: true);

  @override
  Future<MediaImportState> build() async {
    attachDisposeGuard();
  ...
  Future<void> _startStreamAfterBuild() async {
    // AsyncNotifier 的 build 先交付首屏状态，再让 bootstrap 回写 active runs。
  ...
  MediaImportState _mergeFirstPage(
    MediaImportState current,
    MediaImportState result,
  ) {
  ...
  Future<MediaImportState> _fetchFirstPage(MediaImportState base) async {
    try {
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      final jobs = <ImportJobListItemDto>[
        ...current.jobs,
  ...
  Future<void> ensureDetail(int jobId, {bool force = false}) async {
    ...
    try {
      final detail = await ref.read(mediaImportApiProvider).getImportJob(jobId);
      if (isDisposed) return;
      final current = _current;
      final details = Map<int, ImportJobDto>.of(current.details)
        ..[jobId] = detail;
      state = AsyncData(
  ...
  void _upsertDetail(ImportJobDto detail) {
    final current = _current;
    final details = Map<int, ImportJobDto>.of(current.details)
      ..[detail.id] = detail;
  ...
  MediaImportState _replaceJob(
    MediaImportState current,
    ImportJobListItemDto job,
  ) {
    final index = current.jobs.indexWhere((item) => item.id == job.id);
    if (index < 0) return current;
    final jobs = List<ImportJobListItemDto>.of(current.jobs)..[index] = job;
  ...
  Future<String?> _bootstrapStream() async {
    ...
    for (final run in bootstrap.activeTaskRuns) {
      if (run.taskKey == kMediaImportTaskKey) runs[run.id] = run;
    }
  ...
  void _handleStreamEvent(ActivityStreamEvent event) {
    ...
    if (run == null ||
        ...
        run.taskKey != kMediaImportTaskKey) {
      return;
    }
    final current = _current;
  ...
  Future<void> _reconcileFirstPage() async {
    ...
    try {
      ...
      if (isDisposed) return;
      final current = _current;
      final fresh = <int, ImportJobListItemDto>{
        for (final job in response.items) job.id: job,
      };
      final merged = <ImportJobListItemDto>[
        ...response.items,
  ...
  ImportJobListItemDto? _jobForTaskRun(int taskRunId) {
