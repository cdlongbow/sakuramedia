...
| 下载任务状态同步 | 同步 qBittorrent 任务到本地任务表 | 每 5 分钟 |
...
| 以图搜图索引优化 | 维护 Qdrant payload 索引并触发后台优化，不阻塞搜索 | 每天 03:00 |
...
- 选种策略随 `[downloads].preferred_client_kinds` 的**首项**切换两套，详见[常见问题 → 自动下载时，系统怎么选资源？](/faq#auto-download-candidate-selection)
...
任务跑起来之后，具体哪些影片会真正被回刷，由下面这套分层规则决定：

- 从未同步过的影片：先同步一次，之后按下面的分层决定要不要再刷
- 在榜影片（排行榜里还会变动的榜单）：`1` 天间隔，等于每次任务都刷
- 上映时间在最近 `60` 天内：每 `2` 天
- 上映时间在 `60` 到 `180` 天之间：每 `7` 天
- 更早的影片、缺少上映日期的影片：**不再周期性回刷**
- 重新订阅一部影片时，会额外触发一次补刷

::: tip 为什么老片不再周期刷
老片的评分和评论基本不动了，跟着每天刷纯粹浪费抓取额度。JavDB Top250 的历史年份榜单同理——榜单本身已经固定，所以这些影片不算「在榜」。真要更新，重新订阅一次就会补刷。
:::
...
- 同步合集标记
- 用于持续修正哪些影片应被识别成合集
- 判定只看番号特征前缀（`[media].others_number_features`），**不再按影片时长判定**
...
- `movie_interaction_sync_cron`
  保证影片评分、想看、评论等互动统计持续回刷；在榜影片每次都刷，越老的影片刷得越稀，180 天前的老片不再周期刷
- `ranking_sync_cron`
  保证排行榜数据有更新
...
- 通过 `aps <job>` 触发的任务，通常会进入任务中心，方便看进度和结果
