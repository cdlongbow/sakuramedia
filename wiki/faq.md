...
「总览」页的「组件诊断」可以一键实测 DMM 连通性，失败时会给出原因提示。确认不通后优先检查这些点：
...
### 我不想部署 qBittorrent 和 Jackett，能用吗？

结论：能，SakuraMedia 的下载链路是**可选能力**，不配就是不启用，后端不会因此启动失败，其它功能全都能正常工作。

说明：

- 下载器（qBittorrent）和索引器（Jackett）都存在数据库表里，为空就是没配
- 不配的情况下，你**放弃**的是：自动找种、自动下载、下载完自动导入、Jackett 手动搜种
...
- `seeders >= 3`（**115 优先时，BT 候选不看做种人数**，见下一条）
...
<a id="auto-download-candidate-selection"></a>

### 自动下载时，系统怎么选资源？

结论：系统会先过滤掉不合格候选，再按优先级排序，只选最优的一个资源提交下载。**具体用哪套优先级，取决于你在 `[downloads].preferred_client_kinds` 里把哪种下载器排在第一位。**

| 首选下载器 | 候选池 | 池内优先级 | 做种人数门槛 |
|---|---|---|---|
| `qbittorrent`（默认） | 全部可用候选 | `4K > PT > 中字 > 做种数 > 体积` | 所有候选都要求 `seeders >= 3` |
| `cloud115` | 先只看 BT 候选，**BT 池为空才回落**到含 PT 的全池 | `4K > 中字 > 做种数 > 体积` | BT 候选不校验；回落到的 PT 候选仍要求 `seeders >= 3` |

排完序后都会再按标题等字段做一次稳定排序，保证同分候选的结果可复现。

为什么 115 优先时 PT 要降级：

- PT 索引器在设置里就**不允许**绑定 115 离线下载器，所以 PT 候选必然落回本地 qBittorrent，和「让内容进网盘」的意图相反
- 115 离线是云端拉取，本地做种数没有参考价值，而且不少 Jackett 索引器根本不返回 `seeders`（恒为 0），卡门槛会把候选全过滤光

要注意：

- BT/PT 的分层在 `4K` **之外**：115 优先时，哪怕「PT 有 4K、BT 只有普通版」，也会选 BT
- `4K` 是候选池内的一级优先级，高于 `中字`
- 这里说的 `4K` 是下载候选标签，不是本地媒体解析出来的标签
- `[downloads]` 的生效档位是「重启调度器」，改完 `preferred_client_kinds` 需要重启 `aps` 进程才会对这个任务生效
