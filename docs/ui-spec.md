...
- 高级设置 Tab：通过统一 `/config` 维护 media / metadata / scheduler / downloads / logging 五个配置节中的关键字段；下载偏好可在 qBittorrent 与 115 离线之间切换首选顺序，保存后手动搜索/提交立即生效，自动下载任务需重启 APS 调度进程
...
- 失败条目按 `kind` 区分：仅 `file` 在作业终态下提供行内操作，并在顶部提供「重导全部失败」；`skipped` / `warning` / `job` 仅展示原因，不提供操作
