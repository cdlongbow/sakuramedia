...
const Map<String, DiagnosticHint> downloaderConnectivityHints =
  ...
  'cloud115-cookies-invalid': DiagnosticHint(
    cause: '115 登录凭据已失效，当前离线下载入口无法访问对应账号。',
    fixHint: '在媒体库设置中为对应 115 媒体库重新扫码登录，然后重新检测。',
    impact: '无法提交新的 115 离线任务，也无法同步已有离线任务进度。',
    fixTarget: DiagnosticFixTarget.configurationTab(1),
  ),
  'cloud115-upstream-error': DiagnosticHint(
    cause: '115 上游服务暂时不可用，或当前网络无法完成登录状态探测。',
    fixHint: '稍后重试；若持续失败，请检查网络与对应 115 媒体库的登录状态。',
    impact: '115 离线下载与任务进度同步暂时不可用。',
    fixTarget: DiagnosticFixTarget.configurationTab(2),
  ),
...
const Map<String, DiagnosticHint> downloaderStorageHints =
  ...
  'storage-mapping-error': DiagnosticHint(
    cause: 'qBittorrent 保存路径 与本地根路径 映射不通。要么 qB 那边路径不存在，要么容器里挂错了卷。',
    fixHint: '核对「qBittorrent 保存路径」（qB 视角）和「本地根路径」（后端视角）确实指向宿主机的同一个目录。',
    impact: '下载完成后无法归档到媒体库；影片入库流程会一直卡在待处理。',
    fixTarget: DiagnosticFixTarget.configurationTab(2),
  ),
  'hardlink-unsupported': DiagnosticHint(
    cause: '存储不支持硬链接（跨设备、跨文件系统、SMB/NFS/exFAT 之类），导入下载完成的影片会被复制而不是链接。',
    fixHint: '查看wiki快速开始部分： 媒体目录怎么挂才能正确硬链接',
...
String resolveDownloaderConnectivityHintKey(
  ...
  if (type == 'cloud115_cookies_invalid') {
    return 'cloud115-cookies-invalid';
  }
  if (type == 'cloud115_upstream_error') {
    return 'cloud115-upstream-error';
  }
